...
ns_num = 0
il_status = 0
path_conf = './default_medium.conf'
path_new_conf = './default_medium_tmp.conf'
...
def handler():
    ...
    if ns_num!=get_ns_num or il_status!=get_il_status_num:
        ...
        line = fpr.readline()
        while line:
            if 'server 172.24.4.151:5000' in line:
                for cnt in range(get_ns_num):
                    if cnt+1 == get_ns_num:
                        line = '    server 172.24.4.' + str(cnt+1+150) + ':5000 weight=' + str(get_il_status_num) + ';\n'
                    else:
                        line = '    server 172.24.4.' + str(cnt+1+150) + ':5000 weight=' + str(get_il_max) + ';\n'
                    fpw.writelines(line)
                line = fpr.readline()
        ...
        fpw.close()
        r = os.popen("rm /etc/nginx/conf.d/default_medium.conf").readlines()
        r = os.popen("mv " + path_new_conf + " /etc/nginx/conf.d/default_medium.conf").readlines()
...
if __name__ == '__main__':
    loadbalancer.run(host="0.0.0.0", port=8686)
