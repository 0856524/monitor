...
ns_num = 0
il_status = 0
path_conf = './default_high.conf'
path_new_conf = './default_high_tmp.conf'
...
def handler():
    ...
    if ns_num!=get_ns_num or il_status!=get_il_status_num:
        ...
        line = fpr.readline()
        while line:
            if 'server 172.24.4.601:5000' in line:
                for cnt in range(get_ns_num):
                    if cnt+1 == get_ns_num:
                        line = '    server 172.24.4.' + str(cnt+1+600) + ':5000 weight=' + str(get_il_status_num) + ';\n'
                    else:
                        line = '    server 172.24.4.' + str(cnt+1+600) + ':5000 weight=' + str(get_il_max) + ';\n'
                    fpw.writelines(line)
                line = fpr.readline()
            elif 'server 172.24.4.6' in line:
                line = fpr.readline()
            else:
                fpw.writelines(line)
                line = fpr.readline()
        ...
        fpw.close()
        r = os.popen("rm /etc/nginx/conf.d/default_high.conf").readlines()
        r = os.popen("mv " + path_new_conf + " /etc/nginx/conf.d/default_high.conf").readlines()
